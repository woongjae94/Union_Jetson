...
def print_head_pose(angle_yaw, anlge_pitch, angle_roll):
    print(angle_yaw, anlge_pitch, angle_roll)
    if(angle_yaw < -55 ):
        result = 'FarLeft'
    elif(angle_yaw <= -30):
        result = 'Left'
...
if __name__ == "__main__":
    ...
    cam_address = 'http://cam_container:' + args.port + '/?action=snapshot'
        ...
        if (now_time - prev_time) > 200:
            ...
            for idx, det in enumerate(face_detects):
                ...
                if conf > 0.8:
                    ...
                    # 예측 결과 출력
                    print_head_pose(yaw_predicted, pitch_predicted, roll_predicted)
                    prev_time = mills()
                    wait_time = prev_time
            cap.release()
